---
layout: post
title: transformer(1)_introduction
date: 2024-04-08
tags: [transformer]
author: taot
---
...
* [GPT](https://openai.com/research/language-unsupervised) (the Generative Pretrained Transformer)；
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/direct/627616666e40436dbc5dd2c122b78cb2.png#pic_center)
...
![在这里插入图片描述](https://img-blog.csdnimg.cn/direct/17639128b65d4af196d6fcc32113985c.png#pic_center)
...
transformer 模型可以被归纳为以下三类：
![在这里插入图片描述](https://img-blog.csdnimg.cn/direct/e356241ffed04890b9c0e9c8a3cb9526.png#pic_center)
...
* [GPT](https://openai.com/research/language-unsupervised)：结合了 Transformer Decoder 架构和迁移学习，通过根据上文预测下一个单词的预训练任务，在 BookCorpus 数据集上进行了预训练。GPT 模型在分类等下游任务上取得了很好的效果；

* [GPT-2](https://openai.com/research/better-language-models)：受简单且可扩展的预训练方法的启发，OpenAI 通过扩大原始模型和训练集创造了 GPT-2，它能够生成篇幅较长且语义连贯的文本；
...
* [GPT-Neo](https://zenodo.org/records/5297715) / [GPT-J-6B](https://github.com/kingoflolz/mesh-transformer-jax)：由于 GPT-3 没有开源，因此一些旨在重新创建和发布 GPT-3 规模模型的研究人员组成了 EleutherAI，训练出了类似 GPT 的 GPT-Neo 和GPT-J-6B 。当前公布的模型具有 1.3、2.7、60 亿个参数，在性能上可以媲美较小版本的 GPT-3 模型。

#### 5.3 Encoder-Decoder 分支
...
* [BigBird](https://arxiv.org/abs/2007.14062)：由于注意力机制 $O(n^2)$ 的内存要求，Transformer 模型只能处理一定长度内的文本。 BigBird 通过使用线性扩展的稀疏注意力形式，将可处理的文本长度从大多数模型的 512 扩展到 4096，这对于处理文本摘要等需要捕获长距离依赖的任务特别有用。